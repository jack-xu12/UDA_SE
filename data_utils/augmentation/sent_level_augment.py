...
def back_translation(examples, aug_ops, sub_set, aug_copy_num,
  ...
  back_translation_file = "{:s}/github_golden_train_bt_7.csv".format(
      FLAGS.back_translation_dir)
  ...
  bt_data = raw_data_utils.SOSCProcessor._read_tsv(back_translation_file, delimiter=',', quotechar='"')
  try:
    assert len(bt_data) == data_total_size  # without label or exception happens
  except:
      assert len(bt_data)-1 == data_total_size, 'aug data has problem'
      del(bt_data[0])  # delete label data


  # paraphrases = paraphrases[start * text_per_example : end * text_per_example]
  paraphrases = bt_data[start * text_per_example : end * text_per_example]
  aug_examples = []
  aug_cnt = 0
  ...
  for i in range(len(examples)):
      ...
      if text_a == paraphrases[i * text_per_example][0]:
          aug_cnt += 1
      if ori_example.text_b is not None:
          text_b = replace_with_length_check(
              ori_example.text_b,
  ...
  tf.logging.info("applied back translation for {:.1f} percent of preprocessed_data".format(
