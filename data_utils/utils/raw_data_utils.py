...
class DataProcessor(object):
  """Base class for preprocessed_data converters for sequence classification preprocessed_data sets."""
  ...
  def get_labels(self):
    """Gets the list of labels for this preprocessed_data set."""
    raise NotImplementedError()
...
class IMDbProcessor(DataProcessor):
  """Processor for the CoLA preprocessed_data set (GLUE version)."""
...
class SOSCProcessor(DataProcessor):
  ...
  def get_train_examples(self, raw_data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(raw_data_dir, "SOSC_train.csv"),
                       quotechar='"', delimiter=","), "train")

  def get_dev_examples(self, raw_data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(raw_data_dir, "SOSC_test.csv"),
                       quotechar='"', delimiter=","), "test")

  def get_unsup_examples(self, raw_data_dir, unsup_set):
    """See base class."""
    if unsup_set == "unsup_ext":
      return self._create_examples(
          self._read_tsv(os.path.join(raw_data_dir, "SOSC_unsup_ext.csv"),
                         quotechar='"', delimiter=","), "unsup_ext", skip_unsup=False)
    elif unsup_set == "unsup_in":
      return self._create_examples(
          self._read_tsv(os.path.join(raw_data_dir, "SOSC_all.csv"),
                         quotechar='"', delimiter=","), "unsup_in", skip_unsup=False)
...
class AppRProcessor(SOSCProcessor):

    def get_dev_examples(self, raw_data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(raw_data_dir, "AppReviews.csv"),
                           quotechar='"', delimiter=","), "test")

    def get_dev_size(self):
        return 316

class GitHubRProcessor(SOSCProcessor):

    def get_dev_examples(self, raw_data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(raw_data_dir, "github_golden_test_3.csv"),
                           quotechar='"', delimiter=","), "test")

    def get_train_examples(self, raw_data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(raw_data_dir, "github_golden_train_7.csv"),
                           quotechar='"', delimiter=","), "train")
...
class OracleRProcessor(SOSCProcessor):

    def get_dev_examples(self, raw_data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(raw_data_dir, "oracle.csv"),
                           quotechar='"', delimiter=","), "test")
...
class AMAZON2Processor(TextClassProcessor):
  ...
  def get_unsup_examples(self, raw_data_dir, unsup_set):
    ...
    if unsup_set == "unsup_in":
      ...
      unsup_dir = None  # update this path if you use unsupervised preprocessed_data
      return self._create_examples(
          self._read_tsv(
...
def get_processor(task_name):
  ...
  processors = {
      ...
      "sosc": SOSCProcessor,
      "appreviews": AppRProcessor,
      "githubr": GitHubRProcessor,
      "oracler": OracleRProcessor
  }
