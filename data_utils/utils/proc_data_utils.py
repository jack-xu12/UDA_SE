...
def evaluation_input_fn_builder(data_base_path, task, prefetch_size=1000):
  ...
  tf.logging.info("loading eval {} preprocessed_data from these files: {:s}".format(
...
def training_input_fn_builder(
  ...
  if unsup_ratio is not None and unsup_ratio > 0:
    assert aug_ops is not None and aug_copy is not None, \
        "Require aug_ops, aug_copy to load augmented unsup preprocessed_data."
    assert unsup_data_base_path is not None and unsup_data_base_path != "", \
        "Require unsup_data_base_path to load unsup preprocessed_data. Get {}.".format(
  ...
  def input_fn(params):
    ...
    if sup_data_base_path is not None:
      ...
      ## only consider unsupervised preprocessed_data when supervised preprocessed_data is considered
    ...
    # Prefetching creates a buffer to make sure there is always preprocessed_data to
