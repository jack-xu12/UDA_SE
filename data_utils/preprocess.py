...
flags.DEFINE_string(
    "raw_data_dir", None, "Data directory of the raw preprocessed_data")

flags.DEFINE_string(
    "output_base_dir", None, "Data directory of the processed preprocessed_data")
...
flags.DEFINE_string(
    "use_label", "binary", "whether to use Neutral data ['binary', 'multiple']")
...
flags.DEFINE_integer(
    ...
    "we divide the preprocessed_data into three parts, and only process one part"
...
def get_data_for_worker(examples, replicas, worker_id):
  ...
  tf.logging.info("processing preprocessed_data from {:d} to {:d}".format(start, end))
...
def get_data_stats(data_stats_dir, sub_set, sup_size, replicas, examples):
  ...
  if all_exist:
    tf.logging.info("loading preprocessed_data stats from {:s}".format(data_stats_dir))
    data_stats = {}
    for key in keys:
      with tf.gfile.Open(
    ...
    tf.logging.info("dumped preprocessed_data stats to {:s}".format(data_stats_dir))
  return data_stats
...
def convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features_data = {}
    features_data['input_ids'] = input_ids
    features_data['input_mask'] = input_mask
    features_data['input_type_ids'] = input_type_ids
    features_data['label_id'] = label_id
...
class InputFeatures(object):
  """A single set of features of preprocessed_data."""
...
class PairedUnsupInputFeatures(object):
  """Features for paired unsup preprocessed_data."""

  def __init__(self, ori_input_ids, ori_input_mask, ori_input_type_ids,
               aug_input_ids, aug_input_mask, aug_input_type_ids, label_ids):
...
def dump_csv(features_list, data_path, csv_name):

    if not tf.gfile.Exists(data_path):
        tf.gfile.MakeDirs(data_path)
    tf.logging.info("dumping csv_txt preprocessed_data")
    _save_file = os.path.join(data_path, csv_name)

    # get features_columns_name
    _columns_name = [k for k in features_list[0].keys()]
    df = pd.DataFrame(columns = _columns_name)
    for f in features_list:
        df = df.append(f, ignore_index="True")

    df.to_csv(_save_file, index= False, encoding='utf-8')
...
def get_data_by_size_lim(train_examples, processor, sup_size):
  ...
  # Assuming sup_size < number of labeled preprocessed_data and
...
def proc_and_save_sup_data(
  ...
  tf.logging.info("processing preprocessed_data")
  examples = tokenize_examples(examples, tokenizer)

  features, features_list = convert_examples_to_features(
      examples, processor.get_labels(), max_seq_length, tokenizer,
      trunc_keep_right, None, None)

  task_name = processor.__class__.__name__
  task_name = task_name[: task_name.find('Processor')].lower()

  if sub_set == 'train':
    if sup_size == -1: # to get all sup train preprocessed_data for training
      dump_csv(features_list, sup_out_dir, f'{task_name}_sup_train_all.csv')
    else:
      dump_csv(features_list, sup_out_dir, f'{task_name}_sup_train.csv')
  elif sub_set == 'dev':
    dump_csv(features_list, sup_out_dir, f'{task_name}_sup_dev.csv')
...
def proc_and_save_unsup_data(
  ...
  if sub_set == "train":
    ...
    assert False
  # this is the size before spliting preprocessed_data for each worker
  ...
  ori_features, ori_features_list = convert_examples_to_features(
  ...
  aug_features, aug_features_list = convert_examples_to_features(
      aug_examples, labels, max_seq_length, tokenizer,
      trunc_keep_right, data_stats, aug_ops)

  unsup_features_list = []
  for ori_feat, aug_feat in zip(ori_features_list, aug_features_list):
      unsup_features_list.append(
          {
              'ori_input_ids': ori_feat['input_ids'],
              'ori_input_mask': ori_feat['input_mask'],
              'ori_input_type_ids': ori_feat['input_type_ids'],

              'aug_input_ids': aug_feat['input_ids'],
              'aug_input_mask': aug_feat['input_mask'],
              'aug_input_type_ids': aug_feat['input_type_ids'],

              'label_ids': ori_feat['label_id']
  ...
  csv_name = f'{task_name}_unsup_all.csv'
  dump_csv(unsup_features_list, unsup_out_dir, csv_name=csv_name)
...
def main(_):
  ...
  processor.label = FLAGS.use_label
  # Create tokenizer
  ...
  if FLAGS.data_type == "sup":
    ...
    tf.logging.info("Create sup. preprocessed_data: subset {} => {}".format(
    ...
    assert FLAGS.aug_ops is not None, \
        "aug_ops is required to preprocess unsupervised preprocessed_data."
    ...
    tf.logging.info("Create unsup. preprocessed_data: subset {} => {}".format(
    ...
    proc_and_save_unsup_data(
        ...
        FLAGS.worker_id, FLAGS.replicas, FLAGS.task_name)
