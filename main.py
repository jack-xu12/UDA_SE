...
def main(cfg, model_cfg, freeze=False, save_pt=True):
    # Load Configuration
    ...
    if cfg.mode != 'eval':
        if cfg.uda_mode:
            ...
            data_iter = [data.sup_data_iter(), None, data.eval_data_iter()]
    else:
    ...
    #  only update classifier part

    if freeze:
        for name, param in model.named_parameters():
            # freeze transformer part, which is inited from pretrained file
    ...
    # Create trainer
    trainer = train.Trainer(cfg, model, data_iter, optim.optim4GPU(cfg, model), get_device(), save_pt)
    ...
    if cfg.mode == 'eval':
        ...
        _res = torch.cat(results[0]).cpu().numpy()
        _preds = torch.cat(results[1]).cpu().numpy()
        _labels = torch.cat(results[2]).cpu().numpy()
        import numpy as np
        diff_idx = np.where(_preds != _labels)

        import numpy as np
        total_accuracy = np.mean(_res)

        ####### calculate p(+|x) p(-|x) ########

        _x_idx = np.argwhere(_res== 0).squeeze()
        _l = _labels[_x_idx]
        _p = np.where(_l == 1)[0]
        _n = np.where(_l == 0)[0]
        ...
        print(f'distribution p(+|x) --> {p_p_x} p(-|x) --> {p_n_x}' )
